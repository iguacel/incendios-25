...
def render_feature_tile(row, geom, out_svg: Path, out_png: Path,
    ...
    # usamos la misma escala ya calculada (global_scale) y colocamos al centro
    # Para que encaje, recalculamos offsets a partir del bbox de la geometría
    minx, miny, maxx, maxy = geom.bounds
    geom_w, geom_h = (maxx - minx), (maxy - miny)
    # Con la escala global, el tamaño dibujado será:
    draw_w, draw_h = geom_w * global_scale, geom_h * global_scale
    # Centramos en el cuadrado
    ...
    def add_poly(poly: Polygon):
        ...
        dwg.add(dwg.path(
            d=d,
            fill=pick_color(row, cutoff_dt_utc),
            stroke=stroke,
            stroke_width=stroke_width,
            fill_rule="evenodd",
            style="vector-effect:non-scaling-stroke",
