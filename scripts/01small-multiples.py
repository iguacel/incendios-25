...
def make_valid(geom):
    # Compat: si falla make_valid en tu entorno, usa buffer(0)
    try:
...
def compute_area_ha(gdf, area_col_hint="area_ha"):
    """
    Devuelve serie de área en hectáreas. Si existe area_ha válida la usa,
    si no, la calcula en EPSG:3035 (LAEA Europe).
    """
...
def path_from_polygon(poly: Polygon, ox: float, oy: float, scale: float,
    ...
    cmds = ring_to_cmds(ext)
    for cmd, X, Y in cmds:
        if cmd == "Z":
            d_parts.append("Z")
        else:
            d_parts.append(f"{cmd} {fmt(X)} {fmt(Y)}")

    for hole in poly.interiors:
        cmds = ring_to_cmds(list(hole.coords))
        for cmd, X, Y in cmds:
            if cmd == "Z":
                d_parts.append("Z")
            else:
...
def draw_geoms_to_svg(geoms, out_path: Path, cols=14, cell=64, margin=24,
                      fill="#000", stroke="#000", stroke_width=0.4, label=False,
                      font_size=8):
    n = len(geoms)
    if n == 0:
    ...
    for i, geom in enumerate(geoms):
        ...
        bounds = geom.bounds
        w = max(bounds[2] - bounds[0], 1e-9)
        h = max(bounds[3] - bounds[1], 1e-9)
...
def main():
    ap = argparse.ArgumentParser(description="Small multiples de manchas ordenadas por hectáreas (ES 2025).")
    ap.add_argument("--in", dest="inp", required=True, help="GeoJSON de entrada (p.ej. data/ES_2025_fuegos.geojson)")
