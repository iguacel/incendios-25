...
def random_color():
    """Devuelve un color hex aleatorio suave."""
...
def draw_geoms_to_svg_scaled(gdf, out_path: Path, cols=14, cell=64, margin=24,
                             stroke="#000", stroke_width=0.4,
                             label=False, font_size=8):
    """Dibuja geometrías respetando escala global (área relativa) y colorea por provincia."""
    ...
    provinces = gdf["prov"].fillna("NA").unique()
    prov_colors = {prov: random_color() for prov in provinces}
    ...
    for i, geom in enumerate(geoms):
        ...
        prov = gdf.iloc[i].get("prov", "NA")
        fill = prov_colors.get(prov, "#999")
        ...
        if label:
            ...
            area = gdf.iloc[i].get("area_ha_final", None)
            area_txt = f"{area:.0f} ha" if area is not None else "NA"
            ...
            dwg.add(dwg.text(
                f"{mun}",
                insert=(ox_cell + 2, oy_cell + 2 + font_size * 2.2),
                font_size=font_size,
                font_family="MarcinAntB, sans-serif",
                fill="#333"
            ))
            dwg.add(dwg.text(
                f"{ccaa}",
                insert=(ox_cell + 2, oy_cell + 2 + font_size * 3.4),
                font_size=font_size,
                font_family="MarcinAntB, sans-serif",
                fill="#000"
            ))
            dwg.add(dwg.text(
                area_txt,
                insert=(ox_cell + 2, oy_cell + 2 + font_size * 4.6),
                font_size=font_size,
                font_family="MarcinAntB, sans-serif",
                fill="#000"
            ))
...
def main():
    ap = argparse.ArgumentParser(description="Small multiples con escala real, labels y colores por provincia.")
    ...
    ap.add_argument("--label", action="store_true", help="Pinta numeritos + mun + área")
    args = ap.parse_args()
